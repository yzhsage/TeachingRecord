...
>
> **適用版本**：目前 GitHub 專案版本；文件整理日期：2026-08-27。
>
...
| 學生名單 | 新增、改名、指定學校、設定數A／數B或未分組、試聽／轉正、停班、撤銷停班、今天復課、永久刪除；已停班列仍可修正姓名、學校與分組 | `records/classIndex`、`records/studentIndex` | 誤按停班時使用「撤銷停班」保留原學生 ID 與歷史紀錄；真正離班才使用「今天復課」或保留停班狀態；目前班級明確未分組時優先保持空白，不會套用其他班級或全域分組；不要直接永久刪除 |
| 出缺勤 | 依日期記錄出席、請假、曠課、遲到、早退、延課、假期與備註；出席可同時加選遲到／早退 | `records/attendance/<classId>` | 基礎狀態擇一；遲到／早退只有出席時可用；「停班」是學生個人不再進班，除非復課；「延課」是全班該日無法上課 |
| 出缺勤總覽 | A1 儀表板與色彩矩陣、A2 學生行為分析、A3 真正異常時間軸；下方矩陣完整顯示全部有紀錄日，點擊日期 × 學生儲存格可直接修改或清除 | 同出缺勤資料 | 一般請假、曠課、遲到、早退不算異常；只標示停班日／停班後、入班前、全班延課日仍有個人狀態、未知或名單外紀錄；沒有異常時隱藏異常欄位。矩陣以電腦／平板寬版呈現，編輯窗格固定在畫面下方 |
| 平時考 | 新增考試欄位、修改評量名稱、日期、科目、範圍，輸入分數並查看 KPI、分數分布、各次摘要、排行、完整統計與趨勢圖；可依數A／數B／未分組篩選 | `records/quiz/<classId>` | 欄位標題的鉛筆按鈕可同時修改名稱與範圍；修改不會影響既有分數、日期或科目；分組篩選會同步套用到統計與矩陣；成績欄位刪除會連同該欄分數移除，操作前應先備份；色帶以 100 分量尺解讀 |
...
| 備份 | 匯出全部資料 JSON、由檔案或貼上文字匯入 | Firebase 與本機下載檔 | 備份包含手動行事曆事件；匯入前或永久清除學生前一定要先再匯出一份現況備份 |
| 異常恢復 | 找回仍有歷史資料但已從名單移除的學生，或逐位永久清除不需要的資料 | 多個班級資料節點 | 「消失的學生」可重新接回原 ID 並標記為已停班；若資料完全不需要，可查看筆數後二次確認永久清除 |

## 三、日常使用流程

### 1. 每日記錄出缺勤
登入後，首頁預設顯示「行事曆」。選取需要處理的日期，系統會列出當日依課表排定的班級。點選班級卡片進入「出缺勤」，先確認日期與上課時間，再為每位學生選擇狀態，並填寫教學日誌內容與當日備註。狀態變更會自動保存；右上角顯示「儲存中」或「已儲存」。若顯示「儲存失敗，點此重試」，應先按重試並確認網路連線，不要立即關閉頁面。出席、遲到與早退的規則是：先選「出席」，再視需要加選「＋遲到」或「＋早退」，兩者可以同時存在；請假、曠課、延課、假期會排除遲到／早退，且基礎狀態彼此擇一。學生個人若停課，代表不再進班，後續不應再有個人狀態，除非之後復課；延課則代表全班該日無法上課，應視為日期層級狀態。


若當天原本有排課但臨時停課，使用「取消本次上課」並填寫原因；若當天沒有排課但需要補課，使用「新增本次上課（補課）」。這些操作會建立單次課表調整，不會改動原本的週期課表。

### 2. 使用出缺勤總覽與修正早期紀錄
在出缺勤頁切換「總覽」，可以先看儀表板的紀錄日期、出席率、遲到、早退、請假／曠課與待檢視異常日，再以學生行為分析比較個別出席率與異常次數。異常時間軸只會列出真正需要處理的資料：個人停班日或停班後仍有紀錄、入班前仍有紀錄、全班延課／假期日仍有個人狀態、未知狀態或名單外紀錄；一般請假、曠課、遲到、早退，以及全班一致的延課／假期紀錄不會列入異常，沒有異常時整個異常欄位不顯示。下方日期 × 學生狀態矩陣完整顯示所有有紀錄日期，日期欄與學生標頭固定，並以電腦／平板寬版呈現；矩陣儲存格與異常項目都可直接在總覽內開啟固定於畫面下方的修改窗格，不會跳到單日紀錄頁。若原因是停班日／停班後紀錄，窗格會明確提示這裡沒有「停班」狀態，應直接按「清除這筆異常紀錄」；若是誤按停班，則到學生管理按「撤銷停班」以移除停班日期，保留原學生 ID 與歷史紀錄。這些都是單筆操作，系統不會自動批次刪除疑似錯誤資料。修改前若涉及大量早期 Excel 匯入資料，仍應先匯出備份。

### 3. 管理班級與學生

在「所有班級」中可以查看進行中與已封存班級。新建班級時填入班級名稱、科目、年級、可選的分科項目，以及是否需要收費。進入班級後，在「學生與課表」分頁可以修改基本資訊、增加學生、指定學校、設定數A／數B或未分組、設定試聽狀態、停班、撤銷停班或今天復課。分組採學生個別資料，不預設自然組、社會組或其他學校分類；高中二年級以上可依需要指定數A或數B，高中一年級、尚未決定組別或不適用者保留未分組。若同一學生跨班且各班分組不同，畫面以目前班級資料為準；在目前班級選擇「未分組／不適用」後，會保留空白，不會再次帶入其他班級的分組。已停班學生仍可在停班列直接修正姓名、該班學校與分組，適合處理歷史資料顯示錯誤。若是誤按停班後補登回來，應優先使用「撤銷停班」，不要另外新增同名學生。

學生離開班級時，建議使用「停班」而不是「永久刪除」。停班會讓學生不再出現在後續出缺勤名單，但仍保留過往出缺勤、成績與收費資料。若已從名單直接永久刪除，歷史資料仍可能留下學生 ID，並在學生管理上方顯示「消失的學生」。確認資料完全不需要且已完成備份後，可逐位按「永久清除全部紀錄」，先查看出缺勤、平時考、段考與收費筆數，再經二次確認；系統會同步移除本班各資料中的該學生 ID 與 studentIndex 的本班索引，不影響其他學生或其他班級。若想保留歷史資料，請輸入姓名使用「還原」，不要使用永久清除。
...
在班級詳細頁切換到「平時考」或「段考」後，先新增評量欄位，再填寫每位學生分數。學生資料可在「學生與課表」指定數A、數B或未分組／不適用；成績頁上方的分組選單可切換全部分組、數A、數B或未分組／不適用，並同步套用到平均、中位數、最高／最低、及格率、填寫率、分布、排行、趨勢與矩陣。若要更正欄位名稱，至成績矩陣的欄位標題右上角按鉛筆按鈕；平時考編輯器會同時顯示名稱與「範圍（選填）」欄位，輸入新內容後按「儲存」；也可以按 Enter 儲存或按 Escape／「取消」放棄修改。修改名稱或範圍只更新該評量欄位的文字，不會刪除或改動日期、科目、分數及班排／校排資料。平時考可以設定科目與範圍分段；段考可以記錄班排／校排。系統會依目前篩選條件計算平均、中位數、最高／最低、及格率、填寫率、標準差、個別平均、最新分數、前後次變化、班均差、PR 與進步率，並提供五段分數分布、各次評量摘要、個別表現排行與成績趨勢圖。**各次評量的人數分母會依該評量日期判定當時已入班且尚未停班的學生**，因此後來入班或之後停班的學生不會被算進早期考試；各次摘要會顯示「已填／當時人數」，填寫率、各次平均與趨勢也採同一判定。若舊資料沒有評量日期，系統會保留相容行為，以現有名單計算。成績輸入欄會依 90–100、80–89、70–79、60–69、未達 60 顯示色階；這是以 100 分量尺提供直覺提示，若評量有其他滿分，應在解讀時自行換算。
...
| Firebase 路徑 | 內容與主要欄位 |
|---|---|
| `records/classIndex` | 班級陣列；班級包含 `id`、`name`、`subject`、`grade`、`hasFee`、`students`、`subjects`、`scheduleRules`、`overrides`、`archived`；`students` 目前仍保留各班成員與各班進班／停課日期，作為歷史 membership 的權威來源 |
| `records/studentIndex` | 以 `studentId` 為鍵的學生主檔索引；保存姓名、全域學校／年級、各班基本資訊與 `enrollments[<classId>]` 的 `school`、`joinDate`／`endDate`／`resumeDate`。由既有 `classIndex` 相容建立，供歷史畫面在班級欄位缺漏時補回學生基本資料 |
| `records/attendance/<classId>` | 以日期為鍵；每日可包含 `content`、`note`、`records`，其中 `records` 對應 `studentId` 到舊單一字串或新複選陣列狀態；出缺勤資料仍以班級與日期保存，不在匯入時搬動歷史紀錄 |
...
出缺勤狀態的基礎狀態為 `出席`、`請假`、`曠課`、`延課`、`假期`，彼此擇一；`遲到` 與 `早退` 是只有出席才可存在的修飾狀態。舊資料的單一字串仍可讀取，例如舊的 `"遲到"` 會以「出席＋遲到」解讀；新複選資料則保存為狀態陣列。學生個人的「停班」不是每日出缺勤狀態，而是該學生在班級 membership 的 `endDate`；`endDate` 是開始停班的日期，該日即不再列入應出席人數，`date >= endDate` 才算停班後。若之後復課，`resumeDate` 為重新開始計入的日期，且不清除原本的 `endDate` 歷史；若只是誤按停班，則使用學生管理中的「撤銷停班」移除這段日期，而不是建立另一個同名學生。舊匯入學生沒有 `joinDate` 時，出缺勤歷史畫面會以該學生第一筆非空白紀錄作為可推導的起始日，較早日期不會列入點名名單。
全班「延課」則由日期的 `overrides[].action === "cancel"` 與每日一致的 `延課` 紀錄表示。總覽可直接修改或清除單一日期／學生紀錄；若停班日仍有匯入的個人紀錄，應在異常編輯窗格直接清除，而不是填入另一個狀態。清除最後一筆紀錄時會移除該學生的資料鍵，必要時也會移除空白日期節點。歷史畫面顯示學校時，優先順序為該班學生欄位、該班 `enrollments[<classId>].school`、全域 studentIndex 學校。
...
在「備份」面板按「匯出全部資料」會下載 JSON，內容包含班級索引、學生主檔索引、各班級的出缺勤、平時考、段考、收費資料，以及手動行事曆事件。系統也會嘗試把最近備份時間寫入 `records/lastBackupAt`；若這一步失敗，檔案仍可能已下載，但畫面會提示雲端備份時間尚未同步。舊版備份若沒有 `studentIndex`，匯入時會由班級學生資料相容建立，不會因此遺失原有紀錄；既有 `endDate` 也不會因為建立學生主檔而被清除。
...
目前測試涵蓋連續多日事件、不連續日期、多選學校、事件正規化、台灣時區日期，出缺勤舊字串相容、出席與遲到／早退複選互斥、一般出缺勤不誤列異常、全班延課與個人停班／復課後紀錄的異常判定、舊匯入資料起始日推導、總覽統計、學生主檔班別學校優先順序，以及成績數值解析、100 分色帶、分布比例、中位數、前後次變化、入班／停班日期與各次評量容量。
建置成功只代表程式可編譯，不代表 Firebase 登入後的每一條操作路徑都已完成瀏覽器驗證；若修改涉及登入、備份、行事曆表單、成績輸入或 Firebase 寫入，仍應在瀏覽器實際操作一次。
...
| `src/attendance.js` | 出缺勤複選狀態正規化、舊資料相容、互斥規則、真正異常判定與總覽統計 | 修改狀態規則時同步更新 `scripts/attendance.test.mjs` |
| `src/students.js` | 學生主檔索引相容建立、舊匯入起始日推導、各班日期區間保存、停班／復課邊界與歷史基本資料顯示保底 | 修改學生主檔欄位或日期規則時同步更新 `scripts/students.test.mjs` |
