...
| 出缺勤總覽 | A1 儀表板與色彩矩陣、A2 學生行為分析、A3 真正異常時間軸；點擊日期 × 學生儲存格可直接修改或清除 | 同出缺勤資料 | 一般請假、曠課、遲到、早退不算異常；只標示停課後／入班前、全班延課日仍有個人狀態、未知或名單外紀錄 |
...
### 2. 使用出缺勤總覽與修正早期紀錄
在出缺勤頁切換「總覽」，可以先看儀表板的紀錄日期、出席率、遲到、早退、請假／曠課與待檢視異常日，再以學生行為分析比較個別出席率與異常次數。異常時間軸只會列出真正需要處理的資料：個人停課後仍有紀錄、入班前仍有紀錄、全班延課／假期日仍有個人狀態、未知狀態或名單外紀錄；一般請假、曠課、遲到、早退，以及全班一致的延課／假期紀錄不會列入異常。底部仍保留日期 × 學生的橫向狀態矩陣；異常項目會直接顯示學生姓名、原狀態與原因，點擊該筆資料即可跳到指定日期的單日表，並在表格上方開啟該學生的修改窗格。修改前若涉及大量早期 Excel 匯入資料，應先匯出備份；系統不會自動刪除疑似錯誤資料。
