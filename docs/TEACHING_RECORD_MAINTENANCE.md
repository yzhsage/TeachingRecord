...
| 學生名單 | 新增、改名、指定學校、試聽／轉正、停班、重新啟用、永久刪除；已停班列仍可修正姓名與學校 | `records/classIndex`、`records/studentIndex` | 優先使用「停班」保留歷史資料，不要直接永久刪除；跨班學生以該班學校資料優先 |
...
### 2. 使用出缺勤總覽與修正早期紀錄
在出缺勤頁切換「總覽」，可以先看儀表板的紀錄日期、出席率、遲到、早退、請假／曠課與待檢視異常日，再以學生行為分析比較個別出席率與異常次數。異常時間軸只會列出真正需要處理的資料：個人停班日或停班後仍有紀錄、入班前仍有紀錄、全班延課／假期日仍有個人狀態、未知狀態或名單外紀錄；一般請假、曠課、遲到、早退，以及全班一致的延課／假期紀錄不會列入異常，沒有異常時整個異常欄位不顯示。下方日期 × 學生狀態矩陣完整顯示所有有紀錄日期，日期欄與學生標頭固定，並以電腦／平板寬版呈現；矩陣儲存格與異常項目都可直接開啟固定於畫面下方的修改窗格。若原因是停班日／停班後紀錄，窗格會明確提示這裡沒有「停班」狀態，應直接按「清除這筆異常紀錄」；只有確認學生日期設定錯誤時，才改選狀態後儲存。這是單筆操作，系統不會自動批次刪除疑似錯誤資料。修改前若涉及大量早期 Excel 匯入資料，仍應先匯出備份。

### 3. 管理班級與學生

在「所有班級」中可以查看進行中與已封存班級。新建班級時填入班級名稱、科目、年級、可選的分科項目，以及是否需要收費。進入班級後，在「學生與課表」分頁可以修改基本資訊、增加學生、指定學校、設定試聽狀態、停班或重新啟用；已停班學生仍可在停班列直接修正姓名與該班學校，適合處理歷史資料顯示錯誤。
...
出缺勤狀態的基礎狀態為 `出席`、`請假`、`曠課`、`延課`、`假期`，彼此擇一；`遲到` 與 `早退` 是只有出席才可存在的修飾狀態。舊資料的單一字串仍可讀取，例如舊的 `"遲到"` 會以「出席＋遲到」解讀；新複選資料則保存為狀態陣列。學生個人的「停班」不是每日出缺勤狀態，而是該學生在班級 membership 的 `endDate`；`endDate` 是開始停班的日期，該日即不再列入應出席人數，`date >= endDate` 才算停班後。若之後復課，`resumeDate` 為重新開始計入的日期，且不清除原本的 `endDate` 歷史。全班「延課」則由日期的 `overrides[].action === "cancel"` 與每日一致的 `延課` 紀錄表示。總覽可直接修改或清除單一日期／學生紀錄；若停班日仍有匯入的個人紀錄，應在異常編輯窗格直接清除，而不是填入另一個狀態。清除最後一筆紀錄時會移除該學生的資料鍵，必要時也會移除空白日期節點。歷史畫面顯示學校時，優先順序為該班學生欄位、該班 `enrollments[<classId>].school`、全域 studentIndex 學校。
...
目前測試涵蓋連續多日事件、不連續日期、多選學校、事件正規化、台灣時區日期，出缺勤舊字串相容、出席與遲到／早退複選互斥、一般出缺勤不誤列異常、全班延課與個人停班後紀錄的異常判定、總覽統計、學生主檔班別學校優先順序，以及成績數值解析、100 分色帶、分布比例、中位數、前後次變化、入班／停班日期與各次評量容量。建置成功只代表程式可編譯，不代表 Firebase 登入後的每一條操作路徑都已完成瀏覽器驗證；若修改涉及登入、備份、行事曆表單、成績輸入或 Firebase 寫入，仍應在瀏覽器實際操作一次。
...
| `src/students.js` | 學生主檔索引相容建立、各班日期區間保存、停班／復課邊界與歷史基本資料顯示保底 | 修改學生主檔欄位或日期規則時同步更新 `scripts/students.test.mjs` |
