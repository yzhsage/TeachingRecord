...
| 出缺勤 | 依日期記錄出席、請假、曠課、遲到、早退、延課、假期與備註；出席可同時加選遲到／早退 | `records/attendance/<classId>` | 基礎狀態擇一；遲到／早退只有出席時可用；「停課」是學生個人不再進班，除非復課；「延課」是全班該日無法上課 |
| 出缺勤總覽 | A1 儀表板與色彩矩陣、A2 學生行為分析、A3 真正異常時間軸；下方矩陣完整顯示全部有紀錄日，點擊日期 × 學生儲存格可直接修改或清除 | 同出缺勤資料 | 一般請假、曠課、遲到、早退不算異常；只標示停課後／入班前、全班延課日仍有個人狀態、未知或名單外紀錄；沒有異常時隱藏異常欄位 |
...
### 2. 使用出缺勤總覽與修正早期紀錄
在出缺勤頁切換「總覽」，可以先看儀表板的紀錄日期、出席率、遲到、早退、請假／曠課與待檢視異常日，再以學生行為分析比較個別出席率與異常次數。異常時間軸只會列出真正需要處理的資料：個人停課日或停課後仍有紀錄、入班前仍有紀錄、全班延課／假期日仍有個人狀態、未知狀態或名單外紀錄；一般請假、曠課、遲到、早退，以及全班一致的延課／假期紀錄不會列入異常，沒有異常時整個異常欄位不顯示。下方日期 × 學生狀態矩陣完整顯示所有有紀錄日期，日期欄與學生標頭固定；矩陣儲存格與異常項目都可直接開啟指定學生的修改窗格。修改前若涉及大量早期 Excel 匯入資料，應先匯出備份；系統不會自動刪除疑似錯誤資料。
...
| `records/studentIndex` | 以 `studentId` 為鍵的學生主檔索引；保存姓名、學校、年級、各班基本資訊與 `enrollments[<classId>]` 的 `joinDate`／`endDate`／`resumeDate`。由既有 `classIndex` 相容建立，供歷史畫面在班級欄位缺漏時補回學生基本資料 |
...
出缺勤狀態的基礎狀態為 `出席`、`請假`、`曠課`、`延課`、`假期`，彼此擇一；`遲到` 與 `早退` 是只有出席才可存在的修飾狀態。舊資料的單一字串仍可讀取，例如舊的 `"遲到"` 會以「出席＋遲到」解讀；新複選資料則保存為狀態陣列。學生個人的「停課」不是每日出缺勤狀態，而是該學生在班級 membership 的 `endDate`；`endDate` 是開始停課的日期，該日即不再列入應出席人數，`date >= endDate` 才算停課後。若之後復課，`resumeDate` 為重新開始計入的日期，且不清除原本的 `endDate` 歷史。全班「延課」則由日期的 `overrides[].action === "cancel"` 與每日一致的 `延課` 紀錄表示。總覽可直接修改或清除單一日期／學生紀錄，清除最後一筆紀錄時會移除該學生的資料鍵，必要時也會移除空白日期節點。
...
目前測試涵蓋連續多日事件、不連續日期、多選學校、事件正規化、台灣時區日期，出缺勤舊字串相容、出席與遲到／早退複選互斥、一般出缺勤不誤列異常、全班延課與個人停課後紀錄的異常判定、總覽統計，以及成績數值解析、100 分色帶、分布比例、中位數、前後次變化、入班／停班日期與各次評量容量。建置成功只代表程式可編譯，不代表 Firebase 登入後的每一條操作路徑都已完成瀏覽器驗證；若修改涉及登入、備份、行事曆表單、成績輸入或 Firebase 寫入，仍應在瀏覽器實際操作一次。
...
| `src/students.js` | 學生主檔索引相容建立、各班日期區間保存與歷史基本資料顯示保底 | 修改學生主檔欄位或日期規則時同步更新 `scripts/students.test.mjs` |
