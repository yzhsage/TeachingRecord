...
| 學生名單 | 新增、改名、指定學校、設定數A／數B或未分組、試聽／轉正、停班、撤銷停班、今天復課、永久刪除；已停班列仍可修正姓名、學校與分組 | `records/classIndex`、`records/studentIndex` | 誤按停班時使用「撤銷停班」保留原學生 ID 與歷史紀錄；真正離班才使用「今天復課」或保留停班狀態；不要直接永久刪除 |
...
在「所有班級」中可以查看進行中與已封存班級。新建班級時填入班級名稱、科目、年級、可選的分科項目，以及是否需要收費。進入班級後，在「學生與課表」分頁可以修改基本資訊、增加學生、指定學校、設定數A／數B或未分組、設定試聽狀態、停班、撤銷停班或今天復課。分組採學生個別資料，不預設自然組、社會組或其他學校分類；高中二年級以上可依需要指定數A或數B，高中一年級、尚未決定組別或不適用者保留未分組。已停班學生仍可在停班列直接修正姓名、該班學校與分組，適合處理歷史資料顯示錯誤。若是誤按停班後補登回來，應優先使用「撤銷停班」，不要另外新增同名學生。
