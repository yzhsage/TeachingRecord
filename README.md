...
| 點名 | 依上課日期記錄出席、請假、曠課、遲到、早退、延課與假期；出席可同時加選遲到／早退，並保留備註；總覽提供儀表板、學生行為分析、真正異常時間軸與可直接修改的色彩矩陣。異常只標示個人停課後／入班前、全班延課日仍有個人狀態、未知或名單外紀錄 |
...
`npm test` 會驗證日期區間、不連續日期、事件正規化、多選學校、台灣時區日期，以及成績數值解析、分數色帶、分布比例、中位數、前後次變化、入班／停班日期與各次評量歷史人數分母；`npm run build` 會產生 `dist/`。`dist/` 與 `node_modules/` 不應提交至 Git。
