...
在班級的「平時考」與「段考」頁面，成績輸入欄會依 90–100、80–89、70–79、60–69、未達 60 顯示色階；畫面同時提供班級 KPI、分布條、各次評量摘要、個別表現排行，以及含最新、變化、班均差、最高、最低與 PR 的學生統計表。欄位標題右上角的鉛筆按鈕可修改平時考或段考名稱，按「儲存」或 Enter 套用，按 Escape／「取消」放棄；修改名稱不會影響該欄既有分數、日期、科目、範圍或班排／校排。色階以 100 分量尺提供直覺提示，非百分制評量請依實際滿分自行換算後解讀。
