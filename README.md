...
| 區域 | 功能說明 |
|---|---|
| 班級 | 新增、編輯與封存班級，管理學生、學校、年級、分組、課表規則與單次補課；可逐位還原或永久清除已刪名單但仍殘留紀錄的學生；學生目前班級的分組會優先於全域或其他班級資料，且可直接改回數A、數B或未分組；所有主要頁面使用一致的桌機／平板內容寬度 |
| 點名 | 依上課日期記錄出席、請假、曠課、遲到、早退、延課與假期；出席可同時加選遲到／早退，並保留備註；學生學校欄有分組時會顯示為「學校．數A／數B」；總覽提供儀表板、學生行為分析、完整上課情形矩陣與真正異常時間軸。停班日當天起即不再列入應出席；異常只標示停班日／停班後、入班前、全班延課日仍有個人狀態、未知或名單外紀錄。點異常項目或矩陣儲存格會在總覽內開啟固定於畫面下方的編輯窗格；停班日異常可直接清除，不必另填出缺勤狀態；誤按停班可在學生管理中撤銷停班，保留原學生 ID 與歷史資料 |
| 成績 | 管理平時考與段考欄位；提供平均、中位數、最高／最低、及格率、填寫率、分數分布、各次評量摘要、個別排行、班均差與趨勢圖；平時考可修改名稱與範圍，成績可依數A／數B／未分組篩選；段考另支援班排／校排 |
| 收費 | 記錄應收、已收與收款日期 |
| 行事曆 | 顯示國定假日、學測／分科測驗／會考與自訂事件；自訂事件支援不連續日期、連續日期區間、適用學校複選、編輯與刪除 |
| 備份 | 匯出與匯入 JSON；備份包含班級索引、學生主檔索引、教學資料與手動行事曆事件；永久清除學生資料前應先備份 |

上方導覽的「行事曆」會開啟月曆。國定假日以不同底色標示；選定日期沒有節日或事件時，下方事件窗格會隱藏。事件清單會顯示事件名稱、適用學校與多日數量。

若刪除學生後在「學生與課表」上方看到「找到幾位『消失的學生』紀錄」，代表出缺勤、成績或收費資料仍保留該學生 ID。確認不需要這些資料時，可逐位按「永久清除全部紀錄」，先查看出缺勤、平時考、段考與收費筆數，再經二次確認清除；這會同步移除本班各資料中的該學生 ID 與 studentIndex 的本班索引，不會影響其他學生或其他班級。若想保留歷史資料，則應輸入姓名按「還原」，不要使用永久清除。

在班級的「平時考」與「段考」頁面，成績輸入欄會依 90–100、80–89、70–79、60–69、未達 60 顯示色階；畫面同時提供班級 KPI、分布條、各次評量摘要、個別表現排行，以及含最新、變化、班均差、最高、最低與 PR 的學生統計表。各次評量的人數分母會依考試日期判定當時已入班且尚未停班的學生，因此後來入班或之後停班的學生不會被算入早期考試；摘要會顯示「已填／當時人數」，填寫率、各次平均與趨勢也採同一規則。若舊評量沒有日期，則以現有名單維持相容計算。欄位標題右上角的鉛筆按鈕可修改平時考或段考名稱；平時考編輯器會一併提供範圍欄位。按「儲存」或 Enter 套用，按 Escape／「取消」放棄；修改名稱或範圍不會影響該欄既有分數、日期、科目或班排／校排。成績頁的分組選單可切換全部分組、數A、數B或未分組／不適用，會同步更新統計、排行、趨勢與矩陣。學生在目前班級若明確選擇「未分組／不適用」，系統會以目前班級的空白設定為準，不會帶入 studentIndex 或其他班級的數A／數B。色階以 100 分量尺提供直覺提示，非百分制評量請依實際滿分自行換算後解讀。
...
| `src/students.js` | 學生主檔相容建立、班別日期區間、班別專屬學校與分組 fallback |
...
| `scripts/students.test.mjs` | 學生主檔合併、停班／復課日期、班別學校與數A／數B分組優先順序測試 |
...
| `records/studentIndex` | 以學生 ID 為主的基本資料索引，保存姓名、全域學校／年級／分組、各班日期資訊與班別專屬 `enrollments[<classId>].school`／`group`；由舊 `classIndex` 相容建立 |
| `records/attendance/<班級 id>` | 以日期保存點名資料；出缺勤表的學生學校欄會依班別資料顯示，若有分組則接續顯示「．數A」或「．數B」 |
