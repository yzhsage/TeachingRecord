...
| 區域 | 功能說明 |
|---|---|
| 班級 | 新增、編輯與封存班級，管理學生、學校、年級、分組、課表規則與單次補課；可逐位還原或永久清除已刪名單但仍殘留紀錄的學生；所有主要頁面使用一致的桌機／平板內容寬度 |
...
在班級的「平時考」與「段考」頁面，成績輸入欄會依 90–100、80–89、70–79、60–69、未達 60 顯示色階；畫面同時提供班級 KPI、分布條、各次評量摘要、個別表現排行，以及含最新、變化、班均差、最高、最低與 PR 的學生統計表。各次評量的人數分母會依考試日期判定當時已入班且尚未停班的學生，因此後來入班或之後停班的學生不會被算入早期考試；摘要會顯示「已填／當時人數」，填寫率、各次平均與趨勢也採同一規則。若舊評量沒有日期，則以現有名單維持相容計算。欄位標題右上角的鉛筆按鈕可修改平時考或段考名稱；平時考編輯器會一併提供範圍欄位。按「儲存」或 Enter 套用，按 Escape／「取消」放棄；修改名稱或範圍不會影響該欄既有分數、日期、科目或班排／校排。成績頁的分組選單可切換全部分組、數A、數B或未分組／不適用，會同步更新統計、排行、趨勢與矩陣。色階以 100 分量尺提供直覺提示，非百分制評量請依實際滿分自行換算後解讀。
